...
const PropertyImages = ({ property }: { property: any }) => {
    ...
    const imageCaptions = [
        ...
        },
        {
            ...
            desc: 'Fully equipped kitchen with sleek finished and modern appliances.',
        }
    ];

    return (
        <div className='flex max-sm:gap-1 max-md:gap-3 gap-5 h-100 w-full'>
            {property.images.map((pImg: any, index: any) => {
                ...
                return (
                    <div
                        key={index}
                        className={`relative group transition-all duration-500 h-100 overflow-hidden rounded-2xl ${isHovered ? 'grow w-full' : 'max-sm:w-10 max-md:w-20 w-56'}`}
